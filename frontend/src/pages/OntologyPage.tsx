          ...
          <div className="flex items-center gap-10">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-6">
                ...
                <p className="text-[11px] font-black text-content-secondary uppercase tracking-[0.3em] font-mono italic">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            <div key={s.label} className="mcd-card border-white/5 relative overflow-hidden group">
               ...
               <div className="text-[10px] font-black text-content-secondary uppercase tracking-[0.2em] mb-4 italic">{s.label}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="mcd-card border-white/5 bg-surface-base/50">
            <h3 className="text-xl font-black uppercase tracking-tighter mb-8 italic text-content-primary">CONSOLE <span className="text-[#E76F2E]">FILTERS</span></h3>

            {/* Search */}
            <div className="mb-8">
              <label className="text-[10px] font-black text-content-secondary uppercase tracking-[0.2em] block mb-4 ml-1">Entity Intelligence Search</label>
              <div className="relative group/input">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-content-secondary group-focus-within/input:text-[#E76F2E] transition-colors" size={18} />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Query Name..."
                  className="w-full bg-background-200 border-2 border-white/20 rounded-2xl pl-16 pr-6 py-4 text-sm text-content-primary font-black uppercase focus:border-[#E76F2E]/40 transition-all placeholder-[#6B5E57]"
                />
              </div>
            </div>
            ...
            {/* Entity Type Filter */}
            <div className="mb-8">
              <label className="text-[10px] font-black text-content-secondary uppercase tracking-[0.2em] block mb-4 ml-1">Entity Intelligence Type</label>
              <select
            ...
            {/* Domain Filter */}
            <div className="mb-8">
              <label className="text-[10px] font-black text-content-secondary uppercase tracking-[0.2em] block mb-4 ml-1">Intelligence Area</label>
              <select
            ...
            <div className="mb-10">
              <div className="flex items-center justify-between mb-4">
                 <label className="text-[10px] font-black text-content-secondary uppercase tracking-[0.2em] ml-1">Intensity Threshold</label>
              ...
              <input
                ...
                className="w-full accent-[#E76F2E] h-1 bg-surface-base/10 rounded-full appearance-none cursor-pointer"
              />
            </div>
            ...
            <button
              ...
              className="w-full bg-surface-base/5 hover:bg-[#E76F2E]/10 text-content-secondary hover:text-[#E76F2E] font-black py-4 rounded-2xl text-[10px] uppercase tracking-widest transition-all border border-white/5 hover:border-[#E76F2E]/20"
            >
          ...
          {stats && stats.top_entities.length > 0 && (
            <div className="mcd-card border-white/5 bg-surface-base/50">
              <h3 className="text-xl font-black uppercase tracking-tighter mb-8 italic text-content-primary">HIGH <span className="text-[#E76F2E]">DENSITY</span> NODES</h3>
              <div className="space-y-4">
                {stats.top_entities.slice(0, 10).map((entity, idx) => (
                  <div key={entity.id} className="flex items-center justify-between text-[11px] font-black uppercase tracking-tight group hover:translate-x-1 transition-transform cursor-pointer">
                    <span className="text-content-secondary group-hover:text-[#E76F2E] mr-4 font-mono">{String(idx + 1).padStart(2, '0')}</span>
                    <span className="text-content-primary truncate flex-1 group-hover:text-white transition-colors">
                      {entity.name}
                    </span>
                    <span className="text-content-secondary ml-2 font-mono group-hover:text-[#E76F2E] transition-colors">{entity.mention_count}x</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="lg:col-span-3">
          {graphLoading ? (
            <div className="bg-surface-base p-12 rounded-lg border border-white/20 flex items-center justify-center">
              <div className="text-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-400 mx-auto mb-4"></div>
                <p className="text-content-secondary">Loading knowledge graph...</p>
              </div>
            </div>
          ) : filteredNodes && filteredLinks ? (
            <div className="bg-surface-base p-4 rounded-lg border border-white/20">
            ...
            <div className="bg-surface-base p-12 rounded-lg border border-white/20 text-center">
          ...
          {selectedEntity && entityDetails && (
            <div className="mt-6 bg-surface-base p-6 rounded-lg border border-white/20">
              <h3 className="text-2xl font-bold text-slate-100 mb-4">{selectedEntity.name}</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div>
                  <div className="text-sm text-content-secondary">Type</div>
                  <div className="font-medium text-content-primary capitalize">{selectedEntity.entity_type}</div>
                </div>
                <div>
                  <div className="text-sm text-content-secondary">Mentions</div>
                  <div className="font-medium text-content-primary">{selectedEntity.mention_count}</div>
                </div>
                <div>
                  <div className="text-sm text-content-secondary">Sentiment</div>
              ...
              {selectedEntity.description && (
                <div className="mb-6">
                  <div className="text-sm text-content-secondary mb-1">Description</div>
                  <p className="text-content-primary">{selectedEntity.description}</p>
                </div>
              )}

              {selectedEntity.aliases && selectedEntity.aliases.length > 0 && (
                <div className="mb-6">
                  <div className="text-sm text-content-secondary mb-2">Also Known As</div>
                  <div className="flex flex-wrap gap-2">
                    {selectedEntity.aliases.map((alias, idx) => (
                      <span key={idx} className="bg-slate-700 px-3 py-1 rounded-full text-sm text-content-primary">
                        {alias}
                      </span>
                    ))}
                  </div>
                    ...
                    {entityDetails.relationships.slice(0, 10).map((rel: any, idx: number) => (
                      <div key={idx} className="flex items-start gap-3 p-3 bg-slate-700 rounded-lg">
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-content-secondary capitalize">
                            {rel.relationship_type.replace('_', ' ')}
                          </div>
                          {rel.context && (
                            <p className="text-xs text-content-secondary mt-1">{rel.context}</p>
                          )}
                        </div>
